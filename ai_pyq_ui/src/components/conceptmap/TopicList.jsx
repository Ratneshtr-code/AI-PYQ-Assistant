...
export default function TopicList({ topics, organizedTopics, selectedTopic, onSelectTopic, loading, isCollapsed = false }) {
    if (loading) {
        return (
    ...
    // Use organizedTopics if available, otherwise fall back to flat topics
    const displayData = organizedTopics && organizedTopics.length > 0 
        ? organizedTopics 
        : (topics && topics.length > 0 ? [{ chapterTitle: "All Topics", chapterId: "all", topics }] : []);
    ...
    // Flatten all topics for collapsed view
    const allTopicsFlat = displayData.flatMap(chapter => chapter.topics);

    if (isCollapsed) {
        return (
            <div className="h-full overflow-y-auto py-2">
                <div className="space-y-2 px-2">
                    {allTopicsFlat.map((topic, index) => (
                        <motion.button
                            key={topic.id}
    ...
    return (
        <div className="h-full overflow-y-auto">
            <div className="space-y-3 px-2 pb-2">
                {displayData.map((chapter, chapterIndex) => (
                    <div key={chapter.chapterId || chapterIndex} className="space-y-2">
                        {/* Chapter Header */}
                        ...
                        {chapter.topics.map((topic, topicIndex) => {
                            const globalIndex = displayData.slice(0, chapterIndex).reduce((sum, ch) => sum + ch.topics.length, 0) + topicIndex;
                            return (
                                <motion.button
                                    key={topic.id}
                        ...
                        {chapterIndex < displayData.length - 1 && (
                            <div className="py-2">
