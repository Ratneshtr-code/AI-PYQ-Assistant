// src/ExamTopicDistributionPage.jsx
...
export default function ExamTopicDistributionPage() {
    ...
    useEffect(() => {
        ...
        fetch(`${buildApiUrl("dashboard/filters")}?exam=${encodeURIComponent(decodedExamName)}&language=${langParam}`)
            .then((res) => res.json())
            .then((result) => {
                if (result.subjects) {
    ...
    useEffect(() => {
        const updateWidth = () => {
            if (topicChartRef.current) {
                const rect = topicChartRef.current.getBoundingClientRect();
                const width = rect.width || topicChartRef.current.offsetWidth || topicChartRef.current.clientWidth;
                if (width > 0) {
    ...
    useEffect(() => {
        if (!decodedExamName || !selectedSubject) {
            setTopicData([]);
            setLoading(false);
            return;
        }
        ...
        const langParam = language === "hi" ? "hi" : "en";
        let url = `${buildApiUrl("dashboard/topic-weightage")}?exam=${encodeURIComponent(decodedExamName)}&subject=${encodeURIComponent(englishSubjectName)}&language=${langParam}`;
        if (yearFrom) {
    ...
    }, [decodedExamName, selectedSubject, yearFrom, yearTo, language]);
            ...
            >
                ...
                <div className="w-full max-w-7xl mx-auto px-2 md:px-4 lg:px-8 py-4 md:py-6 lg:py-8 space-y-4">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-xl md:text-2xl font-bold text-gray-900 mb-2">
                                    {decodedExamName} {language === "hi" ? "टॉपिक वितरण" : "Topic Distribution"}
                                </h1>
                                <p className="text-xs md:text-sm text-gray-600">
                                    {language === "hi"
                                        ? "विषय-वार टॉपिक वितरण देखें"
                                        : "View topic distribution by subject"}
                                </p>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    {!decodedExamName ? (
                        ...
                        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
                            <p className="text-gray-500 text-center py-8">
                                {language === "hi" ? "कृपया एक विषय चुनें" : "Please select a subject"}
                            </p>
                        </div>
                    ) : loading ? (
                        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
                        ...
                        >
                            <div className="mb-4">
                                <h3 className="text-lg font-semibold text-gray-800 mb-1">
                                    {language === "hi" ? "टॉपिक वितरण" : "Topic Distribution"}
                                </h3>
                                <p className="text-xs text-gray-400 mb-2">
                                    {language === "hi" ? "चयनित विषय के भीतर शीर्ष विषय" : "Top topics within selected subject"}
                                </p>
                            </div>
                            ...
                            {selectedSubject && topicData.length > 0 ? (
                                ...
                                <p className="text-gray-500 text-center py-8">
                                    {language === "hi" ? "इस विषय के लिए कोई टॉपिक डेटा उपलब्ध नहीं है" : "No topic data available for this subject"}
                                </p>
                            ) : (
                                <p className="text-gray-500 text-center py-8">
                                    {language === "hi" ? "विषय चुनने के लिए टॉपिक देखें" : "Select a subject to view topics"}
                                </p>
                            )}
                        </motion.div>
                    )}
